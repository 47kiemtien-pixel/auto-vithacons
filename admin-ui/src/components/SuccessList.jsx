    ...
    <div className="flex flex-col h-full bg-base-100 rounded-3xl shadow-xl overflow-hidden border border-base-content/5">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 terminal-scroll">
        {groups.length === 0 ? (
          ...
          groups.map((url, i) => (
            <div key={url} className="p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-xl flex items-center justify-between gap-3 animate-in zoom-in-95 duration-300">
               <div className="flex items-center gap-3 truncate">
                  <div className="w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center text-white text-[10px] shrink-0">
                    <Icon.Check />
                  </div>
                  <div className="truncate leading-tight">
                    <p className="text-xs font-bold text-emerald-500 truncate capitalize">Thành công</p>
               ...
               <a 
                 ...
                 target="_blank" 
                 rel="noreferrer" 
                 className="p-2 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 transition-all border border-emerald-500/20"
               >
                 <Icon.Globe />
               </a>
            </div>
